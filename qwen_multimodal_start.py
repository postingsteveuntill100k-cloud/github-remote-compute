    ...
    try:
        async with sse_client(url) as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                ...
                print("✅ [Qwen Multimodal] Connected to local MCP server at 127.0.0.1:8000")

                tools_response = await session.list_tools()
                tools_names = [tool.name for tool in tools_response.tools]
                print(f"Available tools for Qwen: {', '.join(tools_names)}")

                # Further integration with the local Qwen model can happen here
